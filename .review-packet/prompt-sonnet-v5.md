...
**Date**: 2026-06-08
**Previous review**: v7.6.1 (Sonnet v4, 2026-06-01)
...
10 commits on main since last review:

| Commit | Description |
|--------|-------------|
| `1921bda` | multi-agent audit quick wins — dust sync, dev mode, GPU layers, perf |
| `8ab589a` | game engine stability — bomb persistence, P2 bot, delta timer bailout, perf |
| `5420573` | council audit — phase guard, privacy crash, score loss, RNG determinism |
| `5eee4b1` | background:#000 inline style to prevent white flash before CSS loads |
| `ad66a16` | statically import English translations to prevent raw key flash on load |
| `28d5b06` | GA method name, enable sourcemaps, init Firebase functions |
| `cce2eb2` | resolve 4 audit findings — CSS class toggle, static import, sourcemap hidden, sentry upload scope |
| `6f31e05` | resolve WARN-007, CRIT-002, INFO-003, INFO-004 — rm secret key, ensureAuth race, dead clientDate, WeakMap comment |
| `89ccf67` | security hardening — local JWT verification, i18n allowlist, log sanitization, gradient cache, querySelectorAll ref pattern, version sync 7.7.0 |
| `cc2fe34` | chore: untrack dev artifacts, embedded git repos, agent files from repo index |

### Key Fixes in v7.7.0
- **Secret key removal**: Hardcoded VITE_GA_SECRET_KEY removed from codebase (commit `6f31e05`)
- **Council audit fixes**: Phase guard added to prevent out-of-phase mutation; privacy settings crash fixed; score loss after gameover prevented; RNG determinism enforced
- **Engine stability**: Bomb persistence across ticks fixed; P2 bot controller patched; delta timer bailout on large gaps; GPU layer management
- **White flash prevention**: `background:#000` inline style on root element
- **i18n flash fix**: English translations statically imported so keys don't flash on load
- **Local JWT verification**: Worker validates JWT locally instead of relying on Firebase Admin SDK for challenge signing
- **Log sanitization**: All logs sanitized to prevent log injection attacks
- **Gradient cache**: Canvas gradient objects cached to avoid GC pressure
...
- Dead code removal (LoadingScreen, ParticleLayer, enableDevMode, resume game)
