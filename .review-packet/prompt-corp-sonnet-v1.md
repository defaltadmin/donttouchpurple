...
1. **Performance** — Core Web Vitals, bundle size, hydration, SSR
2. **Accessibility** — WCAG 2.2 AA compliance, screen reader, keyboard nav
3. **SEO** — Meta tags, structured data, semantic HTML
4. **Security** — CSP, XSS, dependency vulnerabilities
5. **Code Quality** — TypeScript strictness, patterns, maintainability
...
.feature-card:hover {
  transform: translateY(-2px);
  border-color: rgba(253,169,255,0.15);
}
...
.github-link:hover {
  border-color: rgba(253,169,255,0.3);
  transform: translateY(-2px);
}
...
**Total files**: 10 source files (4 config + 2 app + 4 components)
**Total lines**: ~1,574 lines of source code
**Stack**: Next.js 16.2.6, React 19.2.4, TypeScript 5, Tailwind CSS 4, OGL 1.0.11, GSAP 3.15

### Key Architecture Notes

1. **Static export** (`output: "export"` in next.config.ts) -- entire site is pre-rendered HTML
2. **No server components** -- all components are `'use client'`
3. **OGL WebGL** for nebula background (not three.js)
4. **GSAP** for all animations (entrance, scroll-triggered, bot gameplay loop)
5. **Font loading** via Google Fonts `<link>` in layout.tsx (render-blocking)
6. **Bot gameplay loop** runs a 600ms setInterval that auto-taps cells on the landing page
7. **No analytics, no CSP headers, no robots.txt** in this website package
