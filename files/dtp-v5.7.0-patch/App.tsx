...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (lastClaimed !== todayStr) {
      ...
      if (gamesEver > 0) setShowRewardsHub(true);
    }
  ...
  const handleTutorialClose = () => {
    ...
    localStorage.setItem('dtp-games-played', String(next));
    setGamesPlayed(next);
    if (next === 1) setTimeout(() => setShowRewardsHub(true), 1200);
    if (!practiceMode) {
      const newEd = { ...energyData, count: energyData.count - 1 };
      localStorage.setItem(LS_KEYS.ENERGY, JSON.stringify(newEd));
      setEnergyData(newEd);
  ...
  const handleLoginStreakClaim = () => {
    ...
    const newDust = (isNaN(dustRef.current) ? 0 : dustRef.current) + loginStreakReward;
    persistDust(newDust);
    setDust(newDust);
